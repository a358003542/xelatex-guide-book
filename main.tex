...
\addchtoc{第四次修订版前言}
\chapter*{第四次修订版前言}
第四次修订版除了再次全面审视本书籍的内容之外，最重大的一个改动就是将会引入pandoc从tex文件生成epub电子书籍的支持，这就意味着某些LaTex特有的功能将会慢慢抽离出来，这些内容是不可能被epub或者其他标记语言支持的，比如Tikz制图，比如某些奇怪的Tex宏命令，所以本书xelatex基础部分将更加关注于那些latex作为标记语言很容易被其他标记语言转换识别的部分，其他不是那么容易转换甚至不可能转换的部分将会移动到进阶部分而且不会再从tex源码那里进行输出尝试了，只是用文字或者图片进行说明。

关于这部分工作的详细内容整理将会在附录中说明之。目前初步的基调就是将会进一步减少一些花里胡哨的东西，tex定制宏部分将会和文档整合在一起，自定义命令可以用，但是最好基于原Latex原生命令，自定义环境少用，支持性不是很好。而基于原有命令的一些调整修改则基本上是支持的，只是最多在epub那边都是没有区别的——比如你可以定制多种风格的enumerate开头的标签样式，但在epub那边都是\verb+1.+ 这样的样子，如果你希望epub花哨点则需要通过css来调整，就写作和转换来说是没有问题的了。
...
比如说\verb+\num{3d10}+的显示效果就是\num{3d10}，比如说\verb+100\si{\text{厘米}^3}+的显示效果就是100\si{\text{厘米}^3}。
...
\part{xelatex进阶}
\textbf{NOTICE:} 进阶篇这部分内容总的来说是笔者不推荐使用的一些特性，因为他们都太Tex了，前面基础篇的一些内容可能在epub那边一些定制效果会丢失，这还可以接受，最多再epub那边再做出一些css调整，而进阶篇这里的一些很可能会造成你的epub那边内容显示不全设置是整个内容变得混乱不堪。
